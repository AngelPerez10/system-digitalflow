...
- **Órdenes — status `pausado`:** además de `pendiente` / `resuelto`, el status del técnico puede ser `pausado`; exige `motivo_pausa` (texto). El listado agrupa Pendientes → Pausados → Canceladas → Resueltas. **Cancelar** (`cancelada` / `cancelado` en proyectos) es **solo admin** (staff/superuser) en API y UI; exige `motivo_cancelacion`.
