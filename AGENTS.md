...
- **Órdenes / Proyectos — `liquidar` y `cambiar_status`:** permisos especiales (sin bypass de admin) en Gestión de usuarios. `liquidar` marca/desmarca «Liquidado» vía `PATCH …/liquidar/`. Quien solo tiene `liquidar` **no** puede mover el status (aunque tenga `edit`); para ambas acciones hay que activar también `cambiar_status` (`PATCH …/cambiar-status/` si no tiene `edit`).
- **Órdenes / Proyectos — status `saldo_pendiente` («Saldo pendiente»):** trabajo concluido con cobro pendiente. **Solo admin** (staff/superuser) puede poner un registro ahí, igual que cancelar (serializer + `cambiar-status`). Marcar «Liquidado» (`liquidar: true`) **solo** se permite en `saldo_pendiente` (409 en resuelto/cerrado u otro); al liquidar el status pasa a `resuelto` (órdenes) o `cerrado` (proyectos) y queda con la marca Liquidado. Desmarcar (`false`) vale en cualquier status y **no** revierte el status. En órdenes rellena fechas de cierre como `resuelto` y permite enviar el PDF por correo. En listados va entre Pausados y Canceladas (web y móvil); la app móvil lo muestra pero no lo ofrece en el selector del técnico.
- **Órdenes — status `pausado`:** además de `pendiente` / `resuelto`, el status del técnico puede ser `pausado`; exige `motivo_pausa` (texto). El listado agrupa Pendientes → Pausados → Saldo pendiente → Canceladas → Resueltas. **Cancelar** (`cancelada` / `cancelado` en proyectos) es **solo admin** (staff/superuser) en API y UI; exige `motivo_cancelacion`.
...
- **Reporte de mantenimiento**: ruta `/reportes-mantenimiento` (+ `/nuevo`, `/:id`, `/:id/pdf`), módulo `reportes_mantenimiento` (`RequirePermission`; admin bypass). **`own_only`:** por defecto el técnico no-staff solo lista/abre reportes de órdenes donde es `tecnico_asignado` o que él creó; con `own_only=false` (Gestión de usuarios: «Ver también los reportes de otros técnicos») ve todos. Modelo `ReporteMantenimiento` en `apps.operacion`: FK a orden de servicio (`orden_id` / snapshots `orden_folio`, `orden_cliente`) + `foto_orden_url` opcional + secciones JSON `{ id, titulo, fotos_antes[], fotos_despues[] }` (hasta 10 fotos por lado; compat con `foto_antes_url` / `foto_despues_url` legacy). CRUD `GET/POST /api/reportes-mantenimiento/`, `GET/PATCH/DELETE /api/reportes-mantenimiento/{id}/`, `GET /api/reportes-mantenimiento/{id}/pdf/`, `POST /api/reportes-mantenimiento/upload-image/` y `delete-image/` (Cloudinary `reportes-mantenimiento/`; basta `create` o `edit`). Folio `RM-10001+`. PDF generado en servidor (plantilla operativa Antes/Después). FE: `frontend/src/pages/Operacion/ReportesMantenimiento/` (`ReportePdfPage` como Órdenes/Proyectos).
...
- **Push remoto vs Expo Go**: en Android, SDK 53+ `expo-notifications` **tira al importarse** (Expo Go ya no trae FCM: `warnOfExpoGoPushUsage` desde `DevicePushTokenAutoRegistration.fx`). Nunca importar el paquete en el arranque (`SessionProvider`, layouts, `PushProvider`). Usar `pushRemotoDisponible()` + `require()` diferido. En Expo Go la app corre sin avisos; para probar push real: development build (`pnpm build:dev` en `mobile/`). Tests: `src/notifications/__tests__/*.expoGo.test.ts`.
