...
export default function CotizacionEnviarPdfModal({
  ...
  }, [open, cotizacion?.id, initialCorreo, user?.smtp_configured, user?.username]);
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      const resp = await fetchApi(`/api/cotizaciones/${cotizacion.id}/enviar-pdf/`, {
        method: "POST",
  ...
  return (
    ...
    >
      <div className={`${panelClass} relative overflow-hidden`} aria-busy={sending || undefined}>
        ...
        <div className={`relative space-y-5 ${sending ? "pointer-events-none select-none" : ""}`}>
          ...
          <div className="overflow-hidden rounded-xl border border-[#E7E7EA] bg-[#FAFAFA] dark:border-[#273244] dark:bg-[#0f172a]/80">
            <div className="flex items-center justify-between gap-3 border-b border-[#E7E7EA]/80 px-3.5 py-2.5 dark:border-[#273244]">
              ...
              <div className="min-w-0 text-right">
                ...
                <p
                  className="truncate text-sm font-medium text-[#52525B] dark:text-[#e5e7eb]"
                  title={clienteNombre}
                >
                  {clienteNombre}
                </p>
          ...
          >
            <div className="flex items-end justify-between gap-2">
              <Label htmlFor="cotizacion-enviar-pdf-correo">Correo del cliente</Label>
              {!loadingPrefill && (
                <span
            ...
            <Input
              id="cotizacion-enviar-pdf-correo"
              type="email"
              name="correo"
              value={correo}
              onChange={(e) => {
