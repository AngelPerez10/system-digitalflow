...
import { clampPct, formatMoney, toNumber } from "../shared/cotizacionFormUtils";
import {
  ...
  type VisualTableRow,
} from "../shared/cotizacionCategoriasUtils";
...
const tableHeaderClass =
  "sticky top-0 z-10 border-b border-[#E7E7EA] bg-[#FAFAFA]/95 text-[10px] font-semibold uppercase tracking-[0.06em] text-[#52525B] backdrop-blur-sm dark:border-[#273244] dark:bg-[#0f172a]/95 dark:text-[#cbd5e1] sm:text-[11px]";
  ...
  const renderProductRow = (line: CotizacionConceptoLine) => {
    ...
    return (
      ...
      >
        ...
        <td className="px-2 py-2 text-center align-middle font-medium tabular-nums text-[#6E6E77] dark:text-[#8ea0b8]">
          {rowNum}
        </td>
        <td className="whitespace-nowrap px-2 py-2 text-right align-middle font-medium tabular-nums">
          {line.cantidad}
        </td>
        <td className="whitespace-nowrap px-2 py-2 align-middle text-[#52525B] dark:text-[#8EA0B8]">
          {line.unidad || "—"}
        </td>
          ...
          <div className="flex items-center gap-2.5">
            {line.thumbnail_url ? (
              <img
                ...
                alt=""
                className="h-9 w-9 shrink-0 rounded-lg border border-[#E7E7EA] object-cover dark:border-[#273244]"
              />
            ) : (
              <span
                className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-dashed border-[#E7E7EA] bg-[#FAFAFA] text-[10px] font-semibold uppercase text-[#A1A1AA] dark:border-[#273244] dark:bg-[#111827] dark:text-[#64748b]"
                aria-hidden
              >
        ...
        <td className="whitespace-nowrap px-2 py-2 text-right align-middle tabular-nums text-[#6E6E77] dark:text-[#8ea0b8]">
          {clampPct(toNumber(line.descuento_pct, 0)).toFixed(2)}%
        </td>
        <td className="whitespace-nowrap px-2 py-2 text-right align-middle font-semibold tabular-nums text-[#09090B] dark:text-[#f8fafc]">
        ...
        <td className="px-2 py-2 text-center align-middle">
          <div className="inline-flex items-center gap-1">
            <button
              ...
              className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-[#E7E7EA] bg-white text-[#52525B] transition hover:border-[#4B7CFF] hover:text-[#1B5CFF] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#1B5CFF]/30 active:scale-[0.97] dark:border-[#273244] dark:bg-[#111827] dark:text-[#cbd5e1] dark:hover:border-[#1B5CFF] sm:h-8 sm:w-8"
              title="Editar"
            ...
            <button
              ...
              className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-[#E7E7EA] bg-white text-[#52525B] transition hover:border-error-400 hover:text-error-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-error-500/30 active:scale-[0.97] dark:border-[#273244] dark:bg-[#111827] dark:text-[#cbd5e1] dark:hover:border-error-500 sm:h-8 sm:w-8"
              title="Eliminar"
    ...
    return (
      ...
      >
        <td colSpan={10} className="px-3 py-3">
          <p className="text-center text-[11px] italic text-[#A1A1AA] dark:text-[#64748b]">
    ...
    return (
      <tr
        ...
        className={[
          "transition-colors",
          isVirtual
            ? "bg-[#FAFAFA]/80 dark:bg-[#243048]/40"
            : "bg-[#E7E7EA]/90 dark:bg-[#243048]/60",
          isDragging ? "opacity-45" : "",
          isDropTarget ? "ring-2 ring-inset ring-[#1B5CFF]/35" : "",
        ]
          .filter(Boolean)
          .join(" ")}
      >
        <td colSpan={10} className="px-3 py-2.5">
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            ...
            {editingCategoriaId === row.id && !isVirtual ? (
              ...
              <span className="text-xs font-semibold uppercase tracking-[0.06em] text-[#52525B] dark:text-[#cbd5e1] sm:text-[13px]">
                {row.nombre}
              </span>
            )}
            ...
            {!isVirtual && cat && editingCategoriaId !== row.id && (
              <div className="ml-auto flex items-center gap-1">
                <button
                  ...
                  className="inline-flex h-7 w-7 items-center justify-center rounded-md border border-[#E7E7EA] bg-white text-[#6E6E77] transition hover:text-[#1B5CFF] dark:border-[#273244] dark:bg-[#111827] dark:text-[#8ea0b8]"
                ...
                <button
                  ...
                  className="inline-flex h-7 w-7 items-center justify-center rounded-md border border-[#E7E7EA] bg-white text-[#6E6E77] transition hover:border-error-400 hover:text-error-600 dark:border-[#273244] dark:bg-[#111827]"
    ...
    <div className="min-w-0">
      {lines.length > 0 ? (
        <div className="mb-2.5 flex flex-wrap items-center justify-between gap-2 px-0.5 sm:px-1">
          <p className="text-[11px] text-[#6E6E77] dark:text-[#8ea0b8] sm:text-xs">
            <span className="font-medium text-[#52525B] dark:text-[#cbd5e1]">{lines.length}</span>{" "}
            {lines.length === 1 ? "concepto" : "conceptos"}
            {hasCategories ? (
              <>
                {" "}
                · <span className="font-medium text-[#52525B] dark:text-[#cbd5e1]">{categorias.length}</span>{" "}
                {categorias.length === 1 ? "categoría" : "categorías"}
              </>
            ) : null}{" "}
            · arrastra para reordenar
          </p>
          <p className="text-[11px] font-semibold tabular-nums text-[#09090B] dark:text-[#f8fafc] sm:text-xs">
      ...
      <div className="mb-3 rounded-[12px] border border-[#E7E7EA] bg-[#FAFAFA] p-3 dark:border-[#273244] dark:bg-[#1B2539] sm:p-3.5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="flex min-w-0 items-center gap-2.5 sm:max-w-[13rem]">
            ...
            <div className="min-w-0">
              <p className="text-xs font-semibold text-[#09090B] dark:text-[#f8fafc]">Nueva categoría</p>
              <p className="text-[11px] leading-snug text-[#6E6E77] dark:text-[#8ea0b8]">
          ...
          <div className="flex min-w-0 flex-1 items-center gap-2 sm:justify-end">
            ...
            <button
              ...
              className="inline-flex h-9 shrink-0 items-center justify-center gap-1.5 rounded-lg bg-[#1B5CFF] px-3.5 text-xs font-semibold text-white shadow-[0_4px_14px_-6px_rgba(27,92,255,0.45)] transition hover:bg-[#1244D1] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#1B5CFF]/40 disabled:cursor-not-allowed disabled:bg-[#DCE7FF] disabled:text-[#A1A1AA] disabled:shadow-none dark:disabled:bg-[#273244] dark:disabled:text-[#64748b]"
            >
        ...
        {hasCategories ? (
          <div className="mt-2.5 flex flex-wrap items-center gap-1.5 border-t border-[#E7E7EA]/70 pt-2.5 dark:border-[#273244]/80">
            <span className="text-[10px] font-semibold uppercase tracking-[0.08em] text-[#6E6E77] dark:text-[#8ea0b8]">
              Activas
            </span>
      ...
      <Table className="min-w-[820px]">
        ...
        <tbody
          ref={tableBodyRef}
          className="divide-y divide-[#EDEDED] text-[11px] text-[#52525B] dark:divide-[#273244] dark:text-[#e5e7eb] sm:text-[12px]"
        >
          {visualRows.map((row) => {
            ...
            <TableRow>
              <TableCell colSpan={10} className="px-4 py-12 text-center">
                <div className="mx-auto flex max-w-sm flex-col items-center gap-3">
                  <span className="inline-flex h-12 w-12 items-center justify-center rounded-2xl border border-[#E7E7EA] bg-[#FAFAFA] text-[#6E6E77] dark:border-[#273244] dark:bg-[#111827] dark:text-[#8ea0b8]">
                    <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" aria-hidden>
                      <path d="M12 5v14M5 12h14" strokeLinecap="round" />
                    </svg>
                  </span>
                  <div>
                    <p className="text-sm font-medium text-[#09090B] dark:text-[#f8fafc]">Aún no hay conceptos</p>
