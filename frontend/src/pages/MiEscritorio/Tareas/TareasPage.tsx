    ...
    try {
      ...
      tareasFlight = (async () => {
        ...
        if (r.ok) {
          ...
          setTareas(rowsFromResponse<Tarea>(data));
        } else setTareas([]);
      })();
      await tareasFlight;
    } catch (e) {
    ...
    return monitorForElements({
      onDrop: async ({ source, location }) => {
        ...
        try {
          ...
          // No restaurar `prev` stale (puede reaparecer una tarea ya eliminada); forzar refetch
      ...
      >
        <div className="rounded-2xl border border-[#e7ded0] bg-white p-6 shadow-xl dark:border-[#273244] dark:bg-[#111a2b]">
          ...
          <div className="flex justify-end gap-3">
            <button type="button" onClick={delCancel} disabled={deleting} className={orangeBtnOutline}>Cancelar</button>
            <button type="button" onClick={delConfirm} disabled={deleting} className="inline-flex items-center justify-center rounded-xl bg-red-500 px-4 py-2 text-sm font-medium text-white shadow-theme-xs hover:bg-red-600 disabled:opacity-60">{deleting ? "Eliminando…" : "Eliminar"}</button>
          </div>
        </div>
      </Modal>
