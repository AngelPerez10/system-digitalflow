  ...
  return (
    ...
    >
      <div className="flex shrink-0 items-center justify-between gap-2 border-b border-white/10 px-3 py-2.5 pt-[max(0.625rem,env(safe-area-inset-top))] sm:gap-3 sm:px-5 sm:py-3">
        <p className="min-w-0 truncate text-xs font-medium text-[#faf9f5] sm:text-sm">{label}</p>
        <button
          type="button"
          onClick={onClose}
          aria-label="Cerrar vista ampliada"
          className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-white/15 text-[#faf9f5] transition-colors hover:bg-white/10 active:scale-[0.98]"
        >
      ...
      <div className="flex min-h-0 flex-1 items-center justify-center overflow-auto bg-[#0f0e0c] p-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] sm:p-6">
        {url ? (
          <img
            src={url}
            alt={label}
            className="h-auto max-h-[calc(100dvh-3.75rem)] w-full max-w-full object-contain sm:max-h-[min(78vh,900px)] sm:w-auto"
          />
        ) : null}
      </div>
    </Modal>
