...
import { groupProyectosByStatus } from "./proyectoStatusSections";
...
describe("groupProyectosByStatus", () => {
  it("ordena secciones En proceso → Pausados → Cerrados → Otros y omite vacías", () => {
    ...
    expect(sections.map((s) => s.key)).toEqual(["EN_PROCESO", "PAUSADO", "CERRADO", "OTROS"]);
    ...
    expect(sections[3].rows.map((r) => r.id)).toEqual(["3"]);
  });
