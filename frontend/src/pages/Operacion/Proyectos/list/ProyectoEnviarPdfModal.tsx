...
export default function ProyectoEnviarPdfModal({
  open,
  proyecto,
  initialCorreo = "",
  onClose,
    ...
    void (async () => {
      try {
        ...
        const [meRes, correoRes] = await Promise.all([
          mePromise,
          fetchApi(`/api/proyectos/${proyecto.id}/correo-sugerido/`),
        ]);
  ...
  }, [open, proyecto?.id, initialCorreo, user?.smtp_configured, user?.username]);
  ...
  const clienteNombre = String(proyecto?.cliente || "").trim() || "Sin cliente";
  const estadoRaw = String(proyecto?.estado || "").trim() as ProyectoEstado;
  const estadoLabel = estadoProyectoLabel(estadoRaw || "en_proceso");
  const pdfFileName =
    folio && folio !== "—" ? `Proyecto_${folio}.pdf` : proyecto?.id != null ? `Proyecto_${proyecto.id}.pdf` : "Proyecto.pdf";
  ...
  const handleSend = async () => {
    if (!proyecto?.id || sending || smtpBlocked) return;
    ...
    setSending(true);
    try {
      const resp = await fetchApi(`/api/proyectos/${proyecto.id}/enviar-pdf/`, {
        method: "POST",
        ...
        <div className={`relative space-y-5 ${sending ? "pointer-events-none select-none" : ""}`}>
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 pt-0.5">
              <p className={sectionLabelOrangeClass}>{estadoLabel}</p>
              <h2 id={titleId} className={`${erpSubheadingClass} mt-0.5`}>
                Enviar PDF por correo
              </h2>
              <p id={descId} className={`mt-1 text-xs ${claudeBodyClass}`}>
                Revisa el destinatario y envía el reporte del proyecto. Puedes omitir y usar el
                icono de correo más tarde.
              </p>
            </div>
          </div>
          ...
          >
            <div className="flex items-end justify-between gap-2">
              <Label htmlFor="proyecto-enviar-pdf-correo">Correo del cliente</Label>
              {!loadingPrefill && (
                <span
