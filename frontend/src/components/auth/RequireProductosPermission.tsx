...
type ProductosPerm = "view" | "create" | "edit" | "delete";

export default function RequireProductosPermission({
  children,
  required,
}: {
  children: React.ReactNode;
  required: ProductosPerm;
}) {
  ...
  const productos = (permissions as Record<string, unknown>)?.productos ?? {};
  const prodPerms = productos as {
    view?: boolean;
    create?: boolean;
    edit?: boolean;
    delete?: boolean;
  ...
  const allowed =
    required === "view"
      ? prodPerms.view === true
      : required === "create"
        ? prodPerms.create === true
        : required === "edit"
          ? prodPerms.edit === true
          : prodPerms.delete === true;
