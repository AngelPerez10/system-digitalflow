...
function PinIcon({ className }: { className?: string }) {
  return (
    <svg {...pinIconSvgProps} className={className}>
...
export function ClienteDireccionesManager({ clienteId }: Props) {
  const formId = useId();
  const mapContainerId = `direccion-map-${formId.replace(/[^a-zA-Z0-9_-]/g, "")}`;
  ...
  const handleConfirmMap = () => {
    ...
    setDraft((prev) => ({ ...prev, direccion: `https://www.google.com/maps?q=${lat},${lng}` }));
    setShowMapModal(false);
  };
  ...
  return (
    <div className="space-y-3">
      ...
      {loading ? (
        ...
        <p className="rounded-2xl border border-dashed border-[#D3D3D8] bg-white px-3 py-3 text-[13px] text-[#6E6E77] dark:border-[#3A4661] dark:bg-[#111827] dark:text-[#8EA0B8]">
      ...
      {editingId !== null ? (
        <div className="overflow-hidden rounded-[14px] border border-[#E7E7EA] bg-[#FAFAFA] dark:border-[#273244] dark:bg-[#1B2539]">
          <div className="flex items-center gap-2.5 border-b border-[#E7E7EA] bg-white px-3.5 py-2.5 dark:border-[#273244] dark:bg-[#111827]">
            <span className="inline-flex size-7 shrink-0 items-center justify-center rounded-xl bg-[rgba(27,92,255,0.10)] text-[#1B5CFF] dark:bg-[rgba(75,124,255,0.16)] dark:text-[#4B7CFF]">
              <PinIcon className="size-4" />
            </span>
            <p className="text-[13px] font-semibold text-[#09090B] dark:text-[#F8FAFC]">
              {editingId === "new" ? "Nueva dirección" : "Editar dirección"}
            </p>
          </div>
          ...
          <div className="space-y-3 p-3">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              ...
              <div>
                ...
                >
                  ...
                  {draft.is_principal ? "Sí, usar como predeterminada" : "Usar como predeterminada"}
                </button>
              </div>
            </div>
            ...
            <div>
              <div className="flex flex-wrap items-center justify-between gap-2">
                <Label htmlFor={`${formId}-direccion`}>Referencia</Label>
                <button
                  type="button"
                  onClick={() => setShowMapModal(true)}
                  className="mb-1.5 inline-flex h-7 shrink-0 items-center gap-1.5 rounded-full bg-[rgba(27,92,255,0.08)] px-3 text-[12px] font-medium text-[#1B5CFF] transition-colors hover:bg-[rgba(27,92,255,0.14)] dark:bg-[rgba(75,124,255,0.14)] dark:text-[#4B7CFF] dark:hover:bg-[rgba(75,124,255,0.22)]"
                >
            ...
            <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
              <button
                type="button"
                onClick={closeForm}
                className="inline-flex h-10 items-center justify-center rounded-[10px] border border-[#E7E7EA] bg-white px-4 text-[13px] font-medium text-[#09090B] transition-colors hover:bg-[#FAFAFA] dark:border-[#273244] dark:bg-[#111827] dark:text-[#F8FAFC] dark:hover:bg-white/6"
              >
                Cancelar
              </button>
              <button
        ...
        <button
          type="button"
          onClick={openNew}
          className="flex h-11 w-full items-center justify-center gap-1.5 rounded-2xl border border-dashed border-[#1B5CFF]/40 bg-white text-[13px] font-semibold text-[#1B5CFF] transition-colors hover:bg-[rgba(27,92,255,0.06)] dark:border-[#4B7CFF]/40 dark:bg-transparent dark:text-[#4B7CFF] dark:hover:bg-[rgba(75,124,255,0.08)]"
        >
