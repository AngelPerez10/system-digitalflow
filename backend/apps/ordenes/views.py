...
class OrdenViewSet(viewsets.ModelViewSet):
    queryset = Orden.objects.select_related('cliente_id', 'tecnico_asignado', 'creado_por').order_by('-fecha_creacion', '-id')
    ...
    def reindex(self, request):
        ...
        """
        qs = Orden.objects.all().order_by('fecha_creacion', 'id')
...
<html>
  ...
  <body>
    ...
    def pdf(self, request, pk=None):
        ...
        if not html:
            return Response({"detail": "No se pudo generar el PDF. Verifica la configuración de HTMLEDOCS_API_KEY."}, status=500)
